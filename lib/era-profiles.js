...
class EraProfileRegistry {
  constructor(posts, sentences, sourceArchive = archiveSeries) {
    const eraDefinitions = [ { ...module.exports.eraDefinitions[0], sourceUrl: sourceArchive.sourceIndexUrl }, ...sourceArchive.series.map(series => ({ ...series, description: descriptions[series.id] })) ];
    const cacheKey = createHash("sha256").update(JSON.stringify([sourceArchive, posts, sentences])).digest("hex");
    ...
    while (profileCache.size > 2) profileCache.delete(profileCache.keys().next().value);
  }
