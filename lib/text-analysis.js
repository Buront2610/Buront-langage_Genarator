...
function splitSentences(value, options = {}) {
  ...
  const protect = (match) => {
    const index = protectedSpans.push(match) - 1;
    return `\uE000${index}\uE001`;
  };
  const restore = (text) => text.replace(/\uE000(\d+)\uE001/g, (_, index) => (
    protectedSpans[Number(index)] || ""
  ));
  // URLのクエリ`?`やメールアドレス中の記号を文末として分割しない。
  const input = options.corpusCleanup
    ? String(value ?? "")
      .replace(/^\s*(?:>>|＞＞)\s*\d+\s*/gm, "")
      .replace(/^\s*(?:>>|＞＞)\s*[A-Za-zぁ-んァ-ヶ一-龠々ー]{1,20}(?=\s|$)\s*/gm, "")
    : String(value ?? "");
  const source = input
    .replace(/https?:\/\/[A-Za-z0-9._~:/?#[\]@!$&'()*+,;=%-]+/gi, protect)
  ...
  for (const line of source.split(/\n+/)) {
    ...
    for (let index = 0; index < trimmed.length; index += 1) {
      ...
      if (sentence.length >= (options.corpusCleanup ? 2 : 1)) sentences.push(sentence);
      chunk = "";
    }
    const tail = restore(chunk.trim());
    if (tail.length >= (options.corpusCleanup ? 2 : 1)) sentences.push(tail);
  }
  return sentences;
}
...
function extractProtectedValues(value) {
  return protectedOccurrences(value).map(({ raw }) => raw);
}
