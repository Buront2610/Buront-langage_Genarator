...
class ContextNarrativeGenerator {
  constructor(options = {}) {
    this.quoteGrammar = new QuoteGrammar(options.quoteCorpus);
  ...
  candidates(source, options = {}) {
    ...
    const pendingCandidates = this.pendingFrameComposer.compose(model, {
      random,
      quoteContext,
      crisisFor,
    });
    if (pendingCandidates) return pendingCandidates;
