...
const { applyDictionaryOnce } = require("./single-pass-dictionary");
const { createHash } = require("node:crypto");
const { missingProtectedValues } = require("./protected-values");
const { verificationStatus, isPassed, fallbackValidation, finalizeResult } = require("./candidate-results");
const {
  extractFeatures,
  intentTags,
...
function completionAssertions(value) {
  const text = String(value ?? "")
    .replace(/(?:完了|成功|解決|復旧|移設|見学|提出|切り替え|更新|実施|開始|作成|導入|修正|達成)(?:した|済み|できた|している).{0,18}(?:にはならない|わけではない|とは言わない)/g, "")
    .replace(/切り替え(?:た|ている).{0,18}(?:にはならない|わけではない|とは言わない)/g, "")
    .replace(/これで勝つると言うのは実行後/g, "");
  const assertions = new Set();
  const direct = /(完了|成功|解決|復旧|移設|見学|提出|更新|実施|開始|作成|導入|修正|達成)(?:した|済み|できた|している)|(切り替え)(?:た|済み|できた|ている)/g;
  for (const match of text.matchAll(direct)) {
    const type = match[1] || match[2];
    if (type) assertions.add(type);
  }
  if (/終わった|終えた|済ませた|片付いた|片付けた|やり遂げた/.test(text)) assertions.add("完了");
  if (/今の結果が見えた|証明された/.test(text)) assertions.add("結果確定");
  if (/完全\s*解決/.test(text)) assertions.add("解決");
  if (/これで勝つる|勝率は１００％/.test(text)) assertions.add("勝利");
  return assertions;
}
...
class CorpusEngine {
  constructor(options = {}) {
    ...
    this.quoteCorpus = JSON.parse(fs.readFileSync(path.join(dataDirectory, "quote-corpus.json"), "utf8"));
    this.contextNarrative = new ContextNarrativeGenerator({ quoteCorpus: this.quoteCorpus });
    this.faithfulQuoteGrammar = new FaithfulQuoteGrammar(this.corpus.posts, this.contextNarrative, this.quoteCorpus);
    this.eraProfiles = new EraProfileRegistry(this.corpus.posts, this.corpus.sentences, JSON.parse(fs.readFileSync(path.join(dataDirectory, "archive-series.json"), "utf8")));
    ...
    const vectorCacheKey = createHash("sha256").update(fs.readFileSync(corpusPath)).update(fs.readFileSync(seriesPath)).digest("hex");
    ...
    if (!vectorModels) {
      ...
      while (vectorModelCache.size > 2) vectorModelCache.delete(vectorModelCache.keys().next().value);
    }
  ...
  generateCandidates(source, references, level, customRules = [], random = Math.random, eraId = "all") {
    ...
    for (const comparisonCandidate of this.comparisonDrivenCandidates(base, random, variationSource)) {
      ...
      candidate = applyDictionaryOnce(candidate, customRules);
  ...
  semanticScore(source, candidate) {
    ...
    const meaningful = sourceTokens.filter((token) => token.length >= 1);
    ...
    // Intl.Segmenter can split verb stems into a single kanji (急 + いで).
    // Only apply this temporary hard guard to one-character argument candidates.
    // Full POS/dependency and role validation is part of the DocumentIR migration.
    const shortArguments = new Set(Array.from(segmenter.segment(source))
      .filter(({ segment, index }) => /^\p{Script=Han}$/u.test(segment)
        && /^(?:が|は|を)/u.test(source.slice(index + segment.length)))
      .map(({ segment }) => segment));
    const missingShortTerms = meaningful.filter((token) => shortArguments.has(token) && !retained.includes(token));
    const missingProtected = missingProtectedValues(source, candidate);
  ...
  validate(source, candidate, level, eraId = "all") {
    ...
    const sourceCompletionAssertions = completionAssertions(source);
    const candidateCompletionAssertions = completionAssertions(candidate);
    const assertedCompletion = Array.from(candidateCompletionAssertions).some((assertion) => (
      !sourceCompletionAssertions.has(assertion)
    ));
    const assertedPraiseBeforeResolution = /見事な仕事だと感心|どこもおかしくはない/.test(candidate);
    const temporalContextMatch = !(
      faithfulModel.hasFuture && !faithfulModel.hasAchievement && assertedCompletion
      || faithfulModel.hasUnresolved && !faithfulModel.hasAchievement
        && (assertedCompletion || assertedPraiseBeforeResolution)
    );
    ...
    if (semantics.missingShortTerms.length) warnings.push(`保持できなかった一文字内容語: ${semantics.missingShortTerms.join("、")}`);
    ...
    if (!temporalContextMatch) warnings.push("未来または未解決の事実を完了済みとして扱った可能性");

    return {
      total: Math.max(0, Math.min(1, total)),
      semantic: semanticScore,
      contentRetention: semantics.contentRetention,
      missingShortTerms: semantics.missingShortTerms,
      ...
      eraContextMatch,
      warnings,
      checks: [...semanticGuard(source, candidate), {
        code: "protected-occurrences", status: semantics.missingProtected.length ? "fail" : "pass",
        description: "数量の文字表記・符号・出現数、URL・メール・識別子・明示アンカーの原値を照合。数量の意味上の役割は未確認。",
      }, {
        code: "short-content-terms", status: semantics.missingShortTerms.length ? "fail" : "pass",
        description: "一文字の内容語を、他の語の高い一致率で補償しない。",
      }],
      passed: semanticGuard(source, candidate).length === 0
        && !semantics.missingProtected.length
        && !semantics.missingShortTerms.length
        && semanticScore >= 0.62
  ...
  transformSentence(source, options) {
    ...
    const passing = ranked.filter(isPassed);
    ...
    const unusedQuoteCandidates = options.usedFaithfulSignatures && !unusedSelectable.length
      ...
      : [];
    let selected = unusedSelectable[0] || unusedQuoteCandidates[0] || selectable[0] || passing[0];
    let verificationPasses = 1;

    if (!selected || selected.validation.style < 0.42) {
      verificationPasses += 1;
      // Retry from the actual input: a paraphrase may already have lost an actor.
      ...
      if (!selected && isPassed(conservativeResult)) {
        selected = conservativeResult;
      }
    }

    const fallback = !selected;
    if (fallback) selected = { text: source, validation: fallbackValidation() };

    if (options.avoidRepeat && !fallback) {
    ...
    return {
      ...
      alternatives: ranked.slice(0, 3).map((candidate) => ({
        text: candidate.text,
        verificationStatus: verificationStatus(candidate.validation),
  ...
  validateNarrative(source, candidate, level, eraId = "all") {
    ...
    const futureBeatPatterns = [
        /ノンフィクション|おいィ|今からする話/,
        /予定|明日|来週|来月|来年|後日|今後|これから|まだ/,
        /普通は普通|一般人|格の違い/,
        /手順|段取り|準備|実行前|始まっていない|シュミレート/,
        /成功扱いしない|勝利宣言をしない|完了したという事実にはならない|ノンフィクション|一歩引く/,
      ];
    const unresolvedBeatPatterns = [
        /ノンフィクション|おいィ|今からする話/,
        /障害|遅延|処理待ち|原因|できない|決まっていない|未定/,
        /ｓＹレならん|深い悲しみ|ｹﾞｰﾑｵｰﾊﾞｰ|手遅れ/,
        /普通は普通|一般人|格の違い/,
        /未解決|復旧していない|勝利宣言をしない|見事な仕事と言う段階ではない|ノンフィクション|一歩引く/,
      ];
    ...
    const beatPatterns = anchorBeatPatterns[sourceModel.eventFrame]
      || (sourceModel.eventFrame === "future"
        ? futureBeatPatterns
        : sourceModel.hasUnresolved && !sourceModel.hasAchievement
          ? unresolvedBeatPatterns
    ...
    const requiredNarrativeBeats = anchorBeatPatterns[sourceModel.eventFrame]
      ? 5
      : ["observation", "evaluation", "future"].includes(sourceModel.eventFrame) || sourceModel.hasUnresolved
        ? 5
        : sourceModel.hasAchievement ? 5 : 4;
  ...
  convertFullContext(source, options) {
    ...
    const sourceModel = this.contextNarrative.extractModel(source, plainFacts);
    const temporalPending = !sourceModel.hasAchievement
      && (sourceModel.hasFuture || sourceModel.hasUnresolved);
    const rawCandidates = options.era === "all"
      ? baseRawCandidates
      : baseRawCandidates.flatMap((candidate) => this.eraGrammar.candidates(
        candidate,
        options.era,
        options.random,
        { temporalPending },
    ...
    const candidates = Array.from(new Set(rawCandidates.map((rawCandidate) => {
      ...
      candidate = applyDictionaryOnce(candidate, options.customRules);
    ...
    const shuffled = shuffleValues(shortlist.length ? shortlist : passing, options.random);
    ...
    const selected = noveltyPool[0] || localPool[0] || { text: source, validation: fallbackValidation() };
    const fallback = !isPassed(selected);
    ...
    const suggestions = optionCandidates.map((candidate) => ({
      text: candidate.text,
      comparisons: [{ ...comparison, output: candidate.text, fallback: false, validation: candidate.validation, diff: diffTokens(source, candidate.text) }],
  ...
  convert(input, options = {}) {
    if (typeof input !== "string") throw new Error("文章は文字列で指定してください");
    const source = input;
    if (!source.trim()) throw new Error("変換する文章が空です");
    if (/[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/u.test(source)) throw new Error("孤立したサロゲートを含む文章は使用できません");
    if (Array.from(source).length > 5000) throw new Error("文章は5000文字以内にしてください");
    ...
    if (contextMode === "full") {
      return finalizeResult(source, this.convertFullContext(source, { level, customRules, random, avoidRepeat, era }));
    }
    const comparisons = [];
    ...
    for (const paragraph of source.split(/\n{2,}/)) {
      ...
      const paragraphComparisons = sentences.map((sentence, index) => {
        const comparison = this.transformSentence(sentence, {
          ...
          era,
          generationSource: longModel ? rewriteFact(longModel, index, "俺", random).text : sentence,
        });
    ...
    for (let optionIndex = 0; optionIndex < 3; optionIndex += 1) {
      if (!comparisons.length || comparisons.some((comparison) => !comparison.options.some(isPassed))) break;
      const usedSignatures = new Set();
      const chosenParagraphs = comparisonParagraphs.map((paragraph) => paragraph.map((comparison) => {
        const available = comparison.options.filter(isPassed);
        ...
        return { ...comparison, output: selected.text, text: selected.text, validation: selected.validation, fallback: false, diff: diffTokens(comparison.source, selected.text) };
      }));
