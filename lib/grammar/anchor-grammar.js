...
class AnchorGrammar {
  analyze(text, audienceBase = null) {
    ...
    for (const sentence of splitSentences(text)) {
      ...
      const challenger = content.match(/^(.{1,24}?)(?:が|は)(?=.{0,80}(?:勝負|挑ん|挑戦|張り合|ライバル|競争|文句|批判|否定|反対|言い争))/);
      const passiveCritic = content.match(/^(.{1,20}?)(?:から|に)(?=.{0,60}(?:文句|批判|否定|反対|挑発))/);
      const quotedSpeaker = content.match(/^([^「『]{1,20})[「『]/);
      const explanationSource = content.match(/^(.{1,24}?)(?:の説明|の助言|のおかげ|の話|の情報|の指摘)(?=.{0,60}(?:わか|分か|理解|判明|気づ|助か|解決|納得))/);
      const teachingSource = content.match(/^(.{1,20}?)(?:から|に)(?:教え|聞い|説明され|助けられ)/);
      const opinionSource = content.match(/^(.{1,24}?)(?:の意見|の提案|の考え)(?=.{0,50}(?:賛成|同意|納得|正しい|支持))/);
