...
class EraGrammar {
  ...
  candidates(base, eraId, random = Math.random) {
    const frames = this.frames[eraId] || [];
    return shuffle(random, frames).map((frame) => frame.render(base));
  }
