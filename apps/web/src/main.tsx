...
const initial = { task: 'rewrite', contextMode: 'faithful', noveltyMode: 'invent', intensity: 2, series: 'all', backend: 'structured' } as const;
function App() {
  ...
  return <main>
    ...
    <div className="workspace"><section className="editor" aria-labelledby="input-title"><div className="section-top"><h2 id="input-title">01 <span>原文</span></h2><span>{[...source].length.toLocaleString()} / 5,000</span></div>
      ...
      <details><summary>追加辞書</summary><p className="note">1行に「元の語=置き換える語」。事実・数量・引用は保持します。修辞の変更は要確認になる場合があります。</p><label className="sr-only" htmlFor="dictionary">追加辞書</label><textarea id="dictionary" value={dictionary} rows={3} onChange={event => { invalidate(); setDictionary(event.target.value); }} placeholder="寒さ=冷え込み" /></details>
...
function CandidateDetails({ candidate, ir }: { candidate: Candidate; ir: DocumentIR }) {
  ...
  return <div className="candidate-details">
    ...
    <details><summary>原文との対応・検査</summary>
      <p className="legend">緑：原文の保持。青：限定した丁寧形の変更。黄：追加した修辞。紫：別文の引用。</p>
    ...
    <details><summary>語り方・根拠・新規性</summary>
      ...
      <p>S：{candidate.scores.S === null ? '未学習' : candidate.scores.S.toFixed(2)}　Q：{candidate.scores.Q === null ? '未学習' : candidate.scores.Q.toFixed(2)}　読みやすさ：規則検査</p><p>{candidate.novelty.window}</p>
