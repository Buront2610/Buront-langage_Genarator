import { loadAssets, retrievalFor } from '../core/assets';
...
export = function run(input: { warmup: true; assetPath: string } | { result: GenerationResult; analyses: Record<string, Analysis> } | { request: GenerationRequest; analysis: Analysis; assetPath: string; options: GenerationOptions; control: SharedArrayBuffer }) {
  ...
  if ('warmup' in input) { retrievalFor(cachedAssets); return { ready: true }; }
